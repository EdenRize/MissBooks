...
export function EditBook() {
    const [bookToEdit, setBookToEdit] = useState(booksService.getEmptyBook('New Book', 70, 2023, 100, 'en', false))
        ...
        <form onSubmit={onSaveBook}>
            { inputs.map(input=> {
              return <React.Fragment key={input.id}>
                    ...
                    <input required max={input.max}  type={input.type} value={input.value || ''} name={input.name} id={input.id} onChange={handleInputChange} />
                </React.Fragment>
            })}

        <label htmlFor="isOnSale">Is On Sale:  <input checked={isOnSale} onChange={handleInputChange} type="checkbox" id="isOnSale" name="isOnSale" /> </label>
           
        <button>Save Book</button>
        </form>
            </div>
            <Link className="go-back" to="/book"><img src="../assets/img/back.svg"/></Link>
